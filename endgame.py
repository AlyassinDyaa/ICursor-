...
with mp_face_mesh.FaceMesh(max_num_faces=1, refine_landmarks=True, min_detection_confidence=0.5,
    ...
    while True:
        ...
        if results.multi_face_landmarks:  # check if landmarks desired are available or not
            ...
            cv2.circle(frame, center_right, int(l_Radius), (0, 255, 0), 1, cv2.LINE_AA)  # drawing circle on right eye
            cv2.circle(frame, center_left, int(r_Radius), (0, 255, 0), 1, cv2.LINE_AA)  # drawing circle on left eye

            # pyag.moveTo(left_CX, left_CY)
        # **********************************************************************************
